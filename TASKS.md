...
3. [x] #33 Slack bot이 Codex 실행 시 핵심 문서를 자동 컨텍스트로 전달하도록 개선합니다.
4. [ ] #28 EC2에서 필요한 설치 항목과 실행 명령을 구현 결과에 맞게 최신화합니다.

## 1. 백엔드 기반

1. [ ] #3 Fastify API server skeleton과 `/api/health`를 구현합니다.
2. [ ] #5 공통 API envelope과 error response helper를 구현합니다.
...
1. [ ] #18 React app shell과 dashboard navigation을 구현합니다.
2. [ ] #19 백엔드 API client와 DTO handling을 구현합니다.
3. [ ] #20 loading, empty, degraded, error state component를 구현합니다.
4. [ ] #21 table과 filter component를 구현합니다.
5. [ ] #22 cluster summary dashboard를 구현합니다.
